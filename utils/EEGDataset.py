        ...
        for sub_idx, sub_id in enumerate(nsubs):
            ...
            for i in range(eeg_data.shape[0]):
                ...
                self.samples.append((
                    ...
                    img_file,              # image path or file name
...
class EEGDataset(Dataset):
    ...
    def __init__(self, eeg_signals_path, 
        ...
        splits = torch.load(eeg_splits_path, weights_only=True)
        subset_indexes = splits["splits"][0][f"{subset}"]

        loaded = torch.load(eeg_signals_path, weights_only=True)
        ...
        # stddevsfile = "./data/spampinato/eeg/eeg_signals_raw_with_mean_std.pth"
        ...
        for sub_idx in subset_indexes:
            ...
            if subject!=0:
                if loaded['dataset'][sub_idx]['subject']==subject:
                    ...
                    self.subject.append(subject) # zero index
            else:
                ...
                if data_subject not in exclude_subjects:
                    ...
                    self.labels.append(loaded["dataset"][sub_idx]['label'])
                    self.images.append(image_names[loaded["dataset"][sub_idx]['image']])
                    self.subject.append(data_subject) # zero index

        # Compute size
        self.size = len(self.subsetData)

        # zero indexed subject ids
        uniquesubjects = sorted(set(self.subject))  # Sorted for deterministic IDs (optional)
        associate_subject_ids = {subj: idx for idx, subj in enumerate(uniquesubjects)}
        self.subject = [associate_subject_ids[sub] for sub in self.subject]
        ...
        if preprocessin_fn is not None:
            ...
            self.preprocessin_fn = self.transform
    ...
    #         if replace_eeg:
    #             self.subsetData[idx]["eeg"] = f.cpu()
    #         else:
    #             self.image_features.append(f.cpu())
    ...
    def __getitem__(self, i):
        ...
        return eeg,label,image,current_subject,image_features